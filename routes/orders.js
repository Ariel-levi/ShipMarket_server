...
router.patch("/orderPaid", auth, async (req, res) => {
  ...
  try {
    ...
    if (paypalData.status != "COMPLETED") {
      res
        .status(401)
        .json({ err_msg: "There is problem with the transaction" });
    }
    ...
    let data = await OrderModel.updateOne(
      { status: "pending", user_id: req.tokenData._id },
      { status }
...
router.patch("/shipping/takingOrder", auth, async (req, res) => {
  ...
  try {
    let data = await OrderModel.updateOne(
      ...
      { status: "shipped", driver_id: user._id }
    );
    res.json(data);
  } catch (error) {
    return res.status(500).json(error);
...
router.patch("/shipping/orderStatus", auth, async (req, res) => {
  ...
  try {
    let data = await OrderModel.updateOne(
      { _id: orderId, driver_id: user._id },
      { status }
    );
    res.json(data);
  } catch (error) {
